...
(function() {
  var AirPort, WiFiControlSettings, WiFiLog, WiFiScanner, connectionStateMap, execSync, execSyncToBuffer, fs, parseNMCLIOutput, parsePatterns, powerStateMap, win32WirelessProfileBuilder;
  ...
  parseNMCLIOutput = function(output) {
    var connection, connectionKey, j, len, line, matches, pattern, propertyKey, subObj, value;
    output = output.split('\n');
    connection = {};
    pattern = /^([^.]*)\.([^:[]*)(\[(\d)\])?:(.*)/;
    for (j = 0, len = output.length; j < len; j++) {
      ...
      subObj = connection[connectionKey] = connection[connectionKey] || {};
      value = matches[5] !== '--' && matches[5] || null;
      if (matches[4]) {
        subObj[propertyKey] = subObj[propertyKey] || [];
  ...
  module.exports = {
    ...
    connectToAP: function(_ap) {
      var COMMANDS, _msg, com, connectToAPChain, error, error1, error2, error3, error4, errorMsg, i, ifaceState, j, l, len, ref, result, ssid, ssidExist, stdout, xmlContent;
      if (WiFiControlSettings.iface == null) {
        ...
        for (l = 0, len = connectToAPChain.length; l < len; l++) {
          ...
          try {
            ...
            if (process.platform === "linux") {
              ...
              if (errorMsg === ("Error: No network with SSID '" + _ap.ssid + "' found.")) {
    ...
    getIfaceState: function() {
      var KEY, VALUE, _msg, connection, connectionData, connectionName, error, error1, error2, error3, error4, error5, error6, foundInterface, interfaceState, j, k, l, len, len1, len2, ln, ln_trim, m, nmCLIOutput, parsedLine, powerData, ref, ref1, ref2;
      try {
        interfaceState = {};
        switch (process.platform) {
          case "linux":
            ...
            if (interfaceState.power) {
              ...
              if (connectionName) {
                try {
                  ...
                  if (connection.IP4 && connection.IP4.ADDRESS) {
